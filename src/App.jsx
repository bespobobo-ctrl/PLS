        ...
        <div className='min-h-screen bg-[#000] text-white animated-bg pb-32 font-sans'>
            ...
            {view !== 'login' && (
                <div className='fixed bottom-4 left-4 right-4 bg-black/90 backdrop-blur-3xl border border-white/5 p-4 rounded-[2rem] flex justify-around z-50 shadow-2xl'>
                    ...
                    <button onClick={() => setActiveTab('xarita')} className={`flex flex-col items-center gap-1.5 ${activeTab === 'xarita' ? 'text-[#ffcf4b] scale-105' : 'text-white/20'}`}><Monitor size={24} /><span className='text-[8px] font-black uppercase'>Xarita</span></button>
                    <button onClick={() => setActiveTab('bar')} className={`flex flex-col items-center gap-1.5 ${activeTab === 'bar' ? 'text-[#ffcf4b] scale-105' : 'text-white/20'}`}><Boxes size={24} /><span className='text-[8px] font-black uppercase'>Bar</span></button>
                </div>
            )}
