...
import {
    ...
    PauseCircle, Coffee, CheckCircle2, ChevronDown, ChevronUp, Wallet, TrendingDown, ArrowUpRight, BarChart, Boxes, LayoutGrid, Eye, EyeOff, ExternalLink, ListChecks, Info, ChevronLeft, Layout as LayoutIcon, Receipt, LogOut, Terminal, User
} from 'lucide-react';
...
const App = () => {
    ...
    const [clubAdmins, setClubAdmins] = useState(getInitialState('clubAdmins', [
        { name: 'SUPER ADMIN', login: 'SUPER', pass: '9999', club: 'ALL', role: 'SUPER' }
    ]));
    ...
    const [newAdmin, setNewAdmin] = useState({ name: '', login: '', pass: '', club: '', role: 'ADMIN' });
    ...
    }, [rooms, clubAdmins, debts, salesLog, historyEntries, inventory, view, username]);
    ...
    const handleLogin = () => {
        const admin = (clubAdmins || []).find(a => a.login.toUpperCase() === username.toUpperCase() && a.pass === pass);
        if (admin) {
            setUsername(admin.login);
            if (admin.role === 'SUPER') setView('superAdminDashboard');
            else setView('clubDashboard');
            setPass('');
        } else alert('ID yoki Parol noto\'g\'ri!');
    };
    ...
    const confirmCheckout = () => {
        ...
        addToHistory('SESS_CLOSE', checkoutRoom.name, `Seans yakunlandi.`, { startTime: stats.startStr, endTime: endTimeStr, amount: stats.total, duration: stats.time });
    ...
    const renderSuperAdmin = () => (
        <div className='p-6 space-y-6 pb-40 animate-fade-in'>
            <div className='bg-[#ffcf4b]/5 p-8 rounded-[2.5rem] border border-[#ffcf4b]/10'>
                <p className='text-[10px] font-black opacity-20 uppercase tracking-[4px] mb-2'>TIZIM_HOLATI</p>
                <div className='flex justify-between items-end'>
                    <div><h2 className='text-3xl font-black italic gold-text tracking-tighter'>CONTROL_CENTER</h2><p className='text-[8px] opacity-20 font-black uppercase mt-1'>SUPER_ADMIN_MODE</p></div>
                    <div className='w-12 h-12 rounded-2xl bg-[#ffcf4b] flex items-center justify-center text-black shadow-lg shadow-[#ffcf4b]/10'><ShieldCheck size={24} /></div>
                </div>
            </div>
            <div className='space-y-3.5'>
                <p className='text-[9px] font-black opacity-20 uppercase tracking-[4px] px-2'>ADMINLAR BOSHQARUVI</p>
                <button onClick={() => setShowAdminModal(true)} className='w-full py-5 bg-white/5 border border-dashed border-white/10 rounded-2xl text-[10px] font-black uppercase text-[#ffcf4b] active:scale-95 transition-all'>+ YANGI ADMIN QO'SHISH</button>
                <div className='space-y-3'>
                    {(clubAdmins || []).map((admin, idx) => (
                        <div key={idx} className='bg-white/5 border border-white/5 p-5 rounded-2xl flex justify-between items-center'>
                            <div className='flex items-center gap-4'><div className={`w-10 h-10 rounded-xl flex items-center justify-center ${admin.role === 'SUPER' ? 'bg-[#ffcf4b] text-black' : 'bg-white/5 text-white/20'}`}>{admin.role === 'SUPER' ? <Zap size={18} /> : <User size={18} />}</div><div><h4 className='text-[11px] font-black uppercase text-white/80'>{admin.name}</h4><p className='text-[8px] opacity-20 font-black uppercase'>{admin.club} | ID: {admin.login}</p></div></div>
                            {admin.role !== 'SUPER' && <button onClick={() => setClubAdmins(p => p.filter(a => a.login !== admin.login))} className='p-3 text-red-500/20 active:text-red-500 transition-colors'><Trash2 size={16} /></button>}
                        </div>
                    ))}
                </div>
            </div>
    ...
    const renderClubAsosiy = () => (
        <div className='p-6 space-y-5 pb-40 animate-fade-in'>
            ...
            <div className='relative p-7 rounded-[2rem] bg-gradient-to-br from-[#ffcf4b]/10 to-transparent border border-[#ffcf4b]/5 overflow-hidden'>
                ...
                <div className='flex justify-between items-start mb-2'><div><p className='text-[9px] font-black opacity-30 uppercase tracking-[3px] mb-1'>UMUMIY KASSA</p><h2 className='text-4xl font-black italic gold-text tracking-tighter tabular-nums'>{analytics.daily.toLocaleString()} <span className='text-[10px] opacity-20 NOT-italic'>UZS</span></h2></div><div className='text-right'><TrendingUp size={16} className='text-[#ffcf4b] opacity-30 ml-auto mb-1' /><p className='text-[7px] font-black opacity-20 uppercase'>LIVE_DATA</p></div></div>
                <div className='grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-white/5'><div><p className='text-[7px] font-black opacity-20 uppercase mb-1 tracking-widest'>XONA SAVDOSI</p><p className='text-sm font-black text-white/80 tabular-nums'>{analytics.dailyRoom.toLocaleString()}</p></div><div className='text-right'><p className='text-[7px] font-black opacity-20 uppercase mb-1 tracking-widest'>BAR SAVDOSI</p><p className='text-sm font-black gold-text tabular-nums'>{analytics.dailyBar.toLocaleString()}</p></div></div>
            </div>
            ...
            <div className='pt-2 space-y-2.5'><p className='text-[9px] font-black opacity-20 uppercase tracking-[3px] px-1'>JONLI MONITORING</p>{(activeRooms || []).filter(r => r?.isBusy).map(r => { const s = calculateSession(r); return (<div key={r.id} onClick={() => setActiveTab('xarita')} className='bg-white/5 border border-white/5 p-5 rounded-2xl flex justify-between items-center active:scale-[0.98] transition-all cursor-pointer'><div><p className='text-[11px] font-black italic uppercase text-white/80'>{r.name}</p><p className='text-[8px] opacity-20 font-bold mt-0.5'>{s.time} • {s.startStr}</p></div><p className='text-xs font-black gold-text'>{s.total.toLocaleString()} UZS</p></div>); })}</div>
            <div className='bg-red-500/5 border border-red-500/10 p-5 rounded-[2rem]'><div onClick={() => setShowDebtsInAsosiy(!showDebtsInAsosiy)} className='flex justify-between items-center cursor-pointer'><div className='flex items-center gap-3'><Users size={16} className='text-red-500' /><p className='text-[11px] font-black uppercase text-red-500/80'>QARZLAR: {analytics.totalDept.toLocaleString()}</p></div>{showDebtsInAsosiy ? <ChevronUp size={14} className='opacity-20' /> : <ChevronDown size={14} className='opacity-20' />}</div><AnimatePresence>{showDebtsInAsosiy && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className='mt-4 space-y-2.5 overflow-hidden'>{(debts || []).filter(d => d?.club === currentAdminData?.club).reverse().map(d => (<div key={d.id} className='bg-black/40 p-4 rounded-2xl border border-white/5 flex justify-between items-center'><div className='space-y-0.5'><h4 className='text-[10px] font-black uppercase text-white/60'>{d.name}</h4><p className='text-[10px] text-red-500/80 font-black'>-{d.amount.toLocaleString()} UZS</p></div><button onClick={() => { if (window.confirm('Qarz to\'landimi?')) setDebts(p => p.filter(x => x.id !== d.id)) }} className='p-2.5 bg-green-500/5 text-green-500/60 rounded-xl active:scale-90 transition-all'><CheckCircle2 size={14} /></button></div>))}</motion.div>)}</AnimatePresence></div>
        </div>
    );
    ...
    const renderClubHistory = () => {
        ...
        return (
            <div className='p-6 space-y-6 pb-40 animate-fade-in'>
                ...
                <div className='flex gap-2.5 overflow-x-auto no-scrollbar px-1'>{dates.map(dStr => { const dateObj = new Date(dStr); const isSel = selectedHistoryDate === dStr; return (<button key={dStr} onClick={() => setSelectedHistoryDate(dStr)} className={`shrink-0 w-12 h-16 rounded-2xl flex flex-col items-center justify-center transition-all border ${isSel ? 'bg-[#ffcf4b] border-[#ffcf4b] text-black shadow-lg shadow-[#ffcf4b]/10' : 'bg-white/5 border-white/5 text-white/30'}`}><span className='text-[7px] font-black uppercase opacity-60 mb-0.5'>{dateObj.toLocaleDateString('uz-UZ', { weekday: 'short' })}</span><span className='text-sm font-black italic'>{dateObj.getDate()}</span></button>); })}</div>
                <div className='space-y-2.5'>{filteredEntries.map(e => (<div key={e.id} onClick={() => setSelectedHistoryItem(e)} className='bg-white/5 border border-white/5 p-5 rounded-[2rem] flex justify-between items-center active:scale-[0.98] transition-all cursor-pointer'><div className='flex items-center gap-4'><div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${e.type === 'SESS_CLOSE' ? 'bg-[#ffcf4b]/10 text-[#ffcf4b]' : e.type === 'BAR' ? 'bg-green-500/10 text-green-500' : 'bg-white/5 text-white/40'}`}>{e.type === 'SESS_CLOSE' ? <Monitor size={18} /> : e.type === 'BAR' ? <ShoppingCart size={18} /> : <Activity size={18} />}</div><div><h4 className='text-[11px] font-black uppercase text-white/80 italic'>{e.title}</h4><p className='text-[8px] opacity-20 font-black uppercase'>{e.type === 'SESS_CLOSE' ? 'Yopildi' : e.type === 'BAR' ? 'Sotuv' : 'Harakat'}</p></div></div><div className='text-right'>{e.amount ? <p className='text-xs font-black gold-text'>{e.amount.toLocaleString()} UZS</p> : <p className='text-[8px] opacity-20 font-black uppercase'>Batafsil</p>}<p className='text-[8px] opacity-10 font-bold mt-1'>{formatTimeShort(e.timestamp)}</p></div></div>))}</div>
            </div>
        );
    };
    ...
    return (
        <div className='min-h-screen bg-[#000] text-white animated-bg font-sans overflow-x-hidden selection:bg-[#ffcf4b]/20'>
                ...
                {view === 'login' ? (
                    <div className='flex flex-col items-center justify-center min-h-screen p-10'>
                        ...
                        <div className='w-full max-w-[280px] space-y-3.5'>
                            <input type="text" placeholder="ID" className='bg-white/5 border border-white/10 h-14 w-full text-center rounded-2xl text-sm font-black uppercase tracking-widest placeholder:opacity-10 outline-none focus:border-[#ffcf4b]/30 transition-all' value={username} onChange={(e) => setUsername(e.target.value)} />
                            <input type="password" placeholder="PAROL" className='bg-white/5 border border-white/10 h-14 w-full text-center rounded-2xl text-sm font-black uppercase tracking-widest placeholder:opacity-10 outline-none focus:border-[#ffcf4b]/30 transition-all' value={pass} onChange={(e) => setPass(e.target.value)} />
                            <button onClick={handleLogin} className='mt-8 py-4 w-full text-xs font-black uppercase rounded-2xl bg-[#ffcf4b] text-black shadow-lg shadow-[#ffcf4b]/10 active:scale-95 transition-all'>TIZIMGA KIRISH</button>
                        </div>
                    </div>
                ) : (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <header className='px-7 py-5 flex justify-between items-center bg-black/80 backdrop-blur-2xl border-b border-white/5 sticky top-0 z-50'>
                            ...
                            <div className='flex items-center gap-4'>
                                ...
                                <button onClick={() => { if (window.confirm('Chiqasizmi?')) { setView('login'); setUsername(''); localStorage.setItem('view', 'login'); } }} className='p-2 bg-red-500/10 text-red-500/60 rounded-xl active:scale-90 transition-all border border-red-500/10'><LogOut size={16} /></button>
                            </div>
                        </header>
                        <main className='max-w-[480px] mx-auto'>{view === 'superAdminDashboard' ? renderSuperAdmin() : activeTab === 'asosiy' ? renderClubAsosiy() : activeTab === 'bar' ? renderClubBar() : activeTab === 'history' ? renderClubHistory() : renderClubXarita()}</main>
                    </motion.div>
                )}
            </AnimatePresence>

            {/* Toolbar - Only for Club Admins */}
            {view === 'clubDashboard' && (
                        ...
                        {['asosiy', 'xarita', 'bar', 'history'].map((tab) => {
                            ...
                            return (
                                <button key={tab} onClick={() => setActiveTab(tab)} className={`relative flex-1 py-3.5 flex flex-col items-center justify-center gap-1 transition-all duration-300 ${isSel ? 'text-[#ffcf4b]' : 'text-white/20'}`}>
                                    <div className={`transition-all duration-300 ${isSel ? 'scale-110' : 'scale-90 opacity-40'}`}>{tab === 'asosiy' ? <BarChart3 size={20} /> : tab === 'xarita' ? <Monitor size={20} /> : tab === 'bar' ? <Boxes size={20} /> : <History size={20} />}</div>
                                    <span className={`text-[7px] font-black uppercase tracking-[2px] transition-all duration-300 ${isSel ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}`}>{tab.slice(0, 4)}</span>
            ...
            {/* Modals */}
            <AnimatePresence>
                {showAdminModal && (<div className='modal-overlay'><motion.div className='modal-content !p-8 !rounded-[2.5rem]'><h2 className='text-lg font-black italic gold-text text-center mb-8 uppercase'>YANGI ADMIN</h2><div className='space-y-4'><input type="text" placeholder="ADMIN ISMI" className='bg-white/5 border border-white/5 h-14 w-full text-center rounded-2xl text-[10px] font-black uppercase outline-none' value={newAdmin.name} onChange={(e) => setNewAdmin({ ...newAdmin, name: e.target.value })} /><input type="text" placeholder="LOGIN (ID)" className='bg-white/5 border border-white/5 h-14 w-full text-center rounded-2xl text-[10px] font-black uppercase outline-none' value={newAdmin.login} onChange={(e) => setNewAdmin({ ...newAdmin, login: e.target.value })} /><input type="password" placeholder="PAROL" className='bg-white/5 border border-white/5 h-14 w-full text-center rounded-2xl text-[10px] font-black uppercase outline-none' value={newAdmin.pass} onChange={(e) => setNewAdmin({ ...newAdmin, pass: e.target.value })} /><input type="text" placeholder="KLUB NOMI" className='bg-white/5 border border-white/5 h-14 w-full text-center rounded-2xl text-[10px] font-black uppercase outline-none' value={newAdmin.club} onChange={(e) => setNewAdmin({ ...newAdmin, club: e.target.value })} /><div className='flex flex-col gap-2 pt-4'><button onClick={() => { setClubAdmins([...clubAdmins, newAdmin]); setShowAdminModal(false); }} className='py-4.5 bg-[#ffcf4b] text-black font-black text-[10px] uppercase rounded-2xl'>QO'SHISH</button><button onClick={() => setShowAdminModal(false)} className='py-2 text-[9px] opacity-20 font-black uppercase'>BEKOR QILISH</button></div></div></motion.div></div>)}
                {selectedHistoryItem && (<div className='modal-overlay' onClick={() => setSelectedHistoryItem(null)}><motion.div onClick={e => e.stopPropagation()} initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className='modal-content !p-0 !rounded-[2.5rem] !max-w-[85%] border border-white/10 overflow-hidden bg-[#0a0a0a]'><div className='p-8 text-center bg-gradient-to-b from-[#ffcf4b]/10 to-transparent'><div className='w-16 h-16 rounded-[2rem] bg-[#ffcf4b] flex items-center justify-center mx-auto mb-6 shadow-xl shadow-[#ffcf4b]/20'><Receipt size={32} className='text-black' /></div><h2 className='text-xl font-black italic gold-text uppercase tracking-[3px]'>{selectedHistoryItem.title}</h2><p className='text-[9px] opacity-20 font-black uppercase mt-1'>Muvaffaqiyatli seans</p></div><div className='p-8 space-y-6'><div className='grid grid-cols-2 gap-8'><div><p className='text-[8px] font-black opacity-20 uppercase tracking-[2px] mb-1'>OCHILDI</p><p className='text-xs font-black italic text-white/80'>{selectedHistoryItem.startTime || formatTimeShort(selectedHistoryItem.timestamp)}</p></div><div className='text-right'><p className='text-[8px] font-black opacity-20 uppercase tracking-[2px] mb-1'>YOPILDI</p><p className='text-xs font-black italic text-white/80'>{selectedHistoryItem.endTime || formatTimeShort(selectedHistoryItem.timestamp)}</p></div></div>{selectedHistoryItem.duration && (<div className='pt-4 border-t border-white/5 flex justify-between items-center'><p className='text-[8px] font-black opacity-20 uppercase'>DAVOMIYLIGI</p><p className='text-xs font-black text-[#ffcf4b] tabular-nums'>{selectedHistoryItem.duration}</p></div>)}<div className='pt-6 border-t-2 border-dashed border-white/5'><p className='text-[8px] font-black opacity-20 uppercase tracking-[4px] text-center mb-2'>UMUMIY SUMMA</p><p className='text-4xl font-black italic gold-text text-center tracking-tighter'>{selectedHistoryItem.amount ? selectedHistoryItem.amount.toLocaleString() : '0'}<span className='text-[10px] NOT-italic opacity-20 ml-2'>UZS</span></p></div></div><button onClick={() => setSelectedHistoryItem(null)} className='w-full py-6 bg-white/5 border-t border-white/5 text-[9px] font-black uppercase tracking-[3px] text-white/20 active:bg-white/10 transition-all'>Yopish</button></motion.div></div>)}
                {checkoutRoom && (<div className='modal-overlay'><motion.div className='modal-content !p-8 !rounded-[2.5rem] !max-w-[85%] border border-[#ffcf4b]/5'><h2 className='text-xl font-black italic gold-text text-center mb-8 uppercase tracking-[4px]'>YOPISH</h2><div className='bg-[#ffcf4b]/5 p-7 text-center mb-8 rounded-[2rem] border border-[#ffcf4b]/5'><p className='text-[8px] font-black opacity-20 uppercase tracking-[4px] mb-2'>JAMI SUMMA</p><p className='text-4xl font-black italic tracking-tighter tabular-nums gold-text'>{finalStats.total.toLocaleString()}<span className='text-[10px] opacity-20 ml-2'>UZS</span></p></div><div className='space-y-4 mb-8'><input type="number" placeholder="OLINGAN PUL" className='bg-white/5 border border-white/10 h-16 w-full text-2xl font-black text-center outline-none rounded-2xl placeholder:text-[10px]' value={paidAmount} onChange={(e) => setPaidAmount(e.target.value)} />{(finalStats.total - Number(paidAmount) > 0 && Number(paidAmount) > 0) && (<div className='p-5 bg-red-500/5 rounded-2xl border border-red-500/10 space-y-3'><input type="text" placeholder="ISMI" className='bg-white/5 border border-white/5 h-12 w-full text-[10px] px-4 rounded-xl outline-none font-black uppercase' value={debtUser.name} onChange={(e) => setDebtUser({ ...debtUser, name: e.target.value })} /><input type="text" placeholder="TEL" className='bg-white/5 border border-white/5 h-12 w-full text-[10px] px-4 rounded-xl outline-none font-black uppercase' value={debtUser.phone} onChange={(e) => setDebtUser({ ...debtUser, phone: e.target.value })} /></div>)}</div><div className='flex flex-col gap-2'><button onClick={confirmCheckout} className='py-4.5 bg-[#ffcf4b] text-black text-[11px] font-black uppercase rounded-2xl shadow-xl active:scale-95 transition-all'>TASDIQLASH</button><button onClick={() => setCheckoutRoom(null)} className='py-3 text-[9px] opacity-20 font-black uppercase tracking-widest'>BEKOR QILISH</button></div></motion.div></div>)}
                {showAddRoom && (<div className='modal-overlay'><motion.div className='modal-content !p-8 !rounded-[2.5rem] border border-white/5'><h2 className='text-lg font-black italic uppercase gold-text text-center mb-8 tracking-tighter'>SOZLAMALAR</h2><div className='space-y-4'><input type="text" placeholder="XONA NOMI" className='bg-white/5 border border-white/5 h-14 w-full text-center rounded-2xl text-[11px] font-black uppercase outline-none' value={editingRoom ? editingRoom.name : newRoom.name} onChange={(e) => editingRoom ? setEditingRoom({ ...editingRoom, name: e.target.value }) : setNewRoom({ ...newRoom, name: e.target.value })} /><input type="number" placeholder="NARXI (SOATIGA)" className='bg-white/5 border border-white/5 h-14 w-full text-center rounded-2xl text-[11px] font-black uppercase outline-none' value={editingRoom ? editingRoom.price : newRoom.price} onChange={(e) => editingRoom ? setEditingRoom({ ...editingRoom, price: Number(e.target.value) }) : setNewRoom({ ...newRoom, price: Number(e.target.value) })} /><div className='flex flex-col gap-2 pt-4'><button onClick={() => { if (editingRoom) { setRooms(rooms.map(r => r.id === editingRoom.id ? editingRoom : r)); setEditingRoom(null); } else { setRooms([...rooms, { ...newRoom, id: Date.now(), club: currentAdminData.club, isBusy: false, isSuspended: false }]); } setShowAddRoom(false); }} className='py-4.5 bg-[#ffcf4b] text-black font-black text-[10px] uppercase rounded-2xl shadow-xl active:scale-95 transition-all'>SAQLASH</button><button onClick={() => setShowAddRoom(false)} className='py-2 text-[9px] opacity-20 font-black uppercase'>BEKOR QILISH</button></div></div></motion.div></div>)}
