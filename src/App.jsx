...
const App = () => {
    ...
    const analytics = useMemo(() => {
        try {
            ...
            const completedDaily = clubLog.filter(s => (nowMs - (s?.timestamp || 0)) < day).reduce((acc, s) => acc + (s?.amount || 0), 0);
            ...
            const weekly = clubLog.filter(s => (nowMs - (s?.timestamp || 0)) < day * 7).reduce((acc, s) => acc + (s?.amount || 0), 0) + runningRevenue;
            const monthly = clubLog.filter(s => (nowMs - (s?.timestamp || 0)) < day * 30).reduce((acc, s) => acc + (s?.amount || 0), 0) + runningRevenue;
            ...
            console.error(e);
            return { daily: 0, weekly: 0, monthly: 0, runningRevenue: 0, totalDebt: 0, busy: 0, free: 0, suspended: 0 };
        }
    ...
    const confirmCheckout = () => {
        try {
            ...
            setRooms(prev => (prev || []).map(r => r.id === checkoutRoom?.id ? { ...r, isBusy: false, startTime: null, items: [], dailyRevenue: (r.dailyRevenue || 0) + paid, barRevenue: (r.barRevenue || 0) + (stats?.itemsPrice || 0) } : r));
        ...
        <div className='p-4 space-y-6 pb-28'>
            <div className='gold-glass !p-8 bg-gradient-to-br from-[#ffcf4b]/15 to-transparent border-[#ffcf4b]/20'>
                ...
                <div className='flex gap-2 mt-6'>
                    <div className='bg-white/5 px-4 py-2 rounded-xl flex-1 text-center'><p className='text-[8px] opacity-30 font-black uppercase'>Haftalik</p><p className='text-xs font-black italic'>{analytics.weekly.toLocaleString()}</p></div>
                    <div className='bg-white/5 px-4 py-2 rounded-xl flex-1 text-center'><p className='text-[8px] opacity-30 font-black uppercase'>Oylik</p><p className='text-xs font-black italic'>{analytics.monthly.toLocaleString()}</p></div>
                </div>
            </div>
            <div className='space-y-4'>
                <p className='text-[10px] font-black opacity-40 uppercase tracking-widest px-2'>Jonli Monitoring</p>
                <div className='space-y-3'>
                    {(activeRooms || []).filter(r => r?.isBusy).map(r => {
                        const s = calculateSession(r);
                        return (
                            <div key={r.id} onClick={() => { setActiveTab('xarita'); setExpRooms({ [r.id]: true }); }} className='gold-glass !p-5 flex justify-between items-center bg-black/40 border-white/5 active:scale-95 transition-all'>
                                <div className='flex items-center gap-4'><div className='w-10 h-10 rounded-xl bg-[#ffcf4b]/10 flex items-center justify-center text-[#ffcf4b]'><Monitor size={20} /></div><div><h4 className='text-lg font-black italic tracking-tighter uppercase'>{r.name}</h4><p className='text-[9px] font-black gold-text italic tabular-nums'>{s.time}</p></div></div>
                                <div className='text-right'><p className='text-sm font-black tabular-nums'>{s.total.toLocaleString()} UZS</p><p className='text-[8px] opacity-30 font-black uppercase'>Hozirgi_hisob</p></div>
                            </div>
                        );
                    })}
                </div>
            ...
            <div className='gold-glass !p-6 border-red-500/20 bg-red-500/5'>
                <div className='flex justify-between items-center mb-4'><p className='text-[10px] font-black uppercase tracking-widest opacity-60'>Qarzlar ro'yxati</p><p className='text-xs font-black text-red-500'>{analytics.totalDebt.toLocaleString()} UZS</p></div>
                {(debts || []).filter(d => d?.club === currentAdminData?.club).slice(0, 5).map(d => (
                    <div key={d.id} className='flex justify-between items-center p-3.5 bg-black/20 rounded-2xl border border-white/5 mb-2'>
    ...
    const renderClubXarita = () => (
        <div className='p-3 space-y-4 pt-1'>
            <button onClick={() => { setEditingRoom(null); setShowAddRoom(true); }} className='btn-gold-minimal !py-5 bg-[#ffcf4b] text-black border-none font-black text-xs'>YANGI XONA QO'SHISH</button>
                                ...
                                <div className={`p-5 bg-gradient-to-b from-[#ffcf4b]/5 to-transparent transition-all ${isExp ? 'space-y-4' : 'flex justify-between items-center'}`}>
                                    ...
                                    {isExp && (
                                        <div className='pt-6 border-t border-white/5 space-y-5'>
                                            ...
                                            <button onClick={() => { setFinalStats(session); setCheckoutRoom(room); }} className='w-full py-5 bg-red-500 rounded-[1.7rem] text-white font-black uppercase italic text-xs shadow-2xl active:scale-95 transition-all'>STOP VA HISOBLASH</button>
                                        </div>
                                    )}
                                </div>
                            )}
            ...
            <AnimatePresence>
                {checkoutRoom && finalStats && (
                    <div className='modal-overlay'><motion.div initial={{ scale: 0.9, y: 50 }} animate={{ scale: 1, y: 0 }} className='modal-content !p-8 border border-white/5'>
                        <h2 className='text-3xl font-black italic gold-text text-center mb-10 tracking-widest uppercase'>HISOBLASH</h2>
                        <div className='gold-glass p-8 text-center mb-10 bg-[#ffcf4b]/5 border-[#ffcf4b]/20'><p className='text-[10px] opacity-40 uppercase font-black tracking-[4px] mb-2'>JAMI TO'LOV</p><p className='text-5xl font-black italic tracking-tighter tabular-nums'>{(finalStats?.total || 0).toLocaleString()} <span className='text-lg'>UZS</span></p></div>
                        <div className='space-y-4'>
                            ...
                            <button onClick={confirmCheckout} className='btn-gold-minimal !py-8 !rounded-[2.5rem] text-xl font-black shadow-2xl'>TASDIQLASH</button>
                    ...
                    <div className='modal-overlay'><motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className='modal-content !max-w-md'>
                        ...
                        <div className='grid grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-2'>
                            {(inventory || []).map(item => (
                                ...
                                }} className='gold-glass p-6 text-left border-white/5 active:scale-95 transition-all'><p className='text-[10px] font-black uppercase mb-1 opacity-50'>{item.name}</p><p className='text-xl font-black gold-text'>{item.price.toLocaleString()} <span className='text-xs'>UZS</span></p></button>
                            ))}
                        </div>
                    </motion.div></div>
                )}
